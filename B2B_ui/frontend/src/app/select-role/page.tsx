...
export default function SelectRolePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('tutorial-action-completed', {
        detail: { step: 0, action: 'role-selected' }
      }));
      
      // Step 3: Redirect to profile create
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-brand-blue-100 via-brand-purple-50 to-brand-pink-100 dark:from-brand-gray-950 dark:via-brand-gray-900 dark:to-brand-gray-950 flex items-center justify-center p-2 md:p-4 pb-[72px] md:pb-4 relative overflow-hidden">
      {/* Tutorial anchor at top */}
      <div data-tour="role-welcome-top" className="absolute top-4 left-1/2 -translate-x-1/2 w-1 h-1 z-50" />
      ...
      <div className="w-full max-w-4xl z-10">
        ...
        >
          <div className="text-center space-y-2">
            <h1 
              data-tour="role-selection-title"
              className="text-4xl font-bold text-brand-gray-900 dark:text-white bg-gradient-to-r from-brand-gray-900 to-brand-blue-800 bg-clip-text text-transparent dark:bg-none"
            >
          ...
          <div className="grid gap-6 md:grid-cols-2" data-tour="role-options">
            {/* Student Card */}
            <motion.div
              initial={{ opacity: 0, x: -20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 0.2 }}
              data-tour="student-role"
            >
              <Card
